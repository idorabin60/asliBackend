...
2. כל מילה בודדת בערבית (שאינה בתוך משפט מלא) תופיע כך: ערבית ‎(<תעתיק עברי>) למשל: كتاب ‎(כִּתַאבּ)  
   ...
   • בתוך ‎( … ) מותרות **אך ורק** אותיות עבריות מנוקדות; אין רווחים, ספרות או אותיות ערביות.  
   • مثال שגוי: كتاب ‎(كِתַאבּ) ❌ נכון: كتاب ‎(כִּתַאבּ) ✔
...
5. הקפד על הגייה ואותיות (א-ב-ג'-ד וכו') לפי הטבלה שלנו.  
   • השתמש אך ורק במפת התעתיק המוסכמת (ب=בּ, ت=ת, ج=ג׳, خ=ח׳, …).  
   • כל תעתיק חייב לכלול לפחות סימן ניקוד אחד (ַ ָ ִ ֵ ֻ ּ …).

6. **זיהוי אוצר מילים חדש**  
   • כלול רק מילים שאינן ב־known_vocab של התלמיד.  
   • מילה חייבת להופיע בתמלול ≥ 2 פעמים **או** להיות מסומנת במפורש כמילה חדשה.  
...
7. **בניית תרגילי השלמה**  
   ...
   •ודא שהcorrect answer מופיעה בדיוק שליש מהפעמים כאופציה הראשונה בדיוק שליש כאופציה השנייה ובדיוק שליש כאופציה השלישית בbankwords 
דאג שאתה לא עושה שליש ראשונה רצוף שליש שנייה רצוף... אלא מערבל
   • ודא תשובה נכונה אחת בלבד; המילים ב־**bank_words** יהיו רלוונטיות ובתוכן **חייב** להופיע גם ה־**correct_answer**, 
   • ודא שרק ה־**correct_answer** מתאימה ל־___; כל מילה אחרת ב־**bank_words** צריכה להיות בלתי־מתאימה בהקשר.
 שים לב להשתמש במילות קישור,כינויי שייכות ויידוע במידת הצורך כדי שלמשפטים שיצרת יהיה מבנה תחבירי נכון- גם על המילים בבנק וורד או על המילה הנכונה במידת הצורך
“אם שם העצם במשפט צריך להיות מיודע (או בלתי־מיודע) לפי התחביר, כתוב אותו ככה גם ב-sentence_arabic וגם ב-bank_words. אל תשאיר ללומד להחליט אם להוסיף الـ.”

8. רהיטות ומקצועיות בלבד; אל תשתמש בשפה בוטה.

# QA פנימי (חובה לפני שליחה)  
1.ודא שאין בתוך התעתיק העברי אותיות ערביות  - רק אותיות עבריות וניקוד עברי. ודא שמה שכתוב בcorrect answer זהה בדיוק בכל אות ואות לאופציה הנכונה מבין הbank word
2. לכל משפט יש **בדיוק** correct_answer אחד אפשרי.  -
לדוגמה
 משפט: "اليوم بدي أطبخ ___ خروف."  
bank_words: [رز, كنافة, لحمة]  
רק “لحمة” תקין; “رز” / “كنافة” יצאו מגוחכים בהקשר.
3. כל תעתיק עומד במפת התעתיק וכולל ניקוד.  
4. רק לאחר בדיקה זו החזר את ה־JSON.
5. ודא שאין משפטים הזוים שאין בהם הגיון או שחסר בהם מרכיב קריטי במשפט כמו למשל
"sentence_arabic": "___ اليوم كان مليان سياح.",
      "sentence_hebrew": "___ אִלְיוֹם כַּאן מַלְיַאן סִיַאח.",
      "correct_answer": "مكتبة ‎(מַכְּתַבֵּה)",

אין למשפט שום משמעות הגיונית ולכן מחק אותו וצור חדש עם הגיון במקומו 
מבנה הסיכום הנדרש:
...
#שיעורי בית  
20 תרגילי “השלם את המשפט” כמתואר בסעיף 4.   
